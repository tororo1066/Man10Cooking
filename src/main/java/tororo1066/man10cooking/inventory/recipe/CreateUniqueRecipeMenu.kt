...
class CreateUniqueRecipeMenu(val isEdit: Boolean, val data: UniqueRecipe? = null) : SInventory(SJavaPlugin.plugin, "CreateUniqueRecipe", 5) {
    ...
    override fun renderMenu(p: Player): Boolean {
        ...
        if (data != null){
            data.ingredients.forEach {
                craftingSlots.forEach second@ { i ->
                    ...
                    this.inv.setItem(i,SItem(it.infoItemStack?:ItemStack(Material.BOOK))
                        .addLore("§cクリックで削除")
                        .setCustomData(SJavaPlugin.plugin, "ingredient", PersistentDataType.STRING, it.internalName))
                    return@forEach
                }
            }
            data.ingredientCategories.forEach {
                craftingSlots.forEach second@ { i ->
                    if (getItem(i) != null)return@second
        ...
        setItem(16, SInventoryItem(Material.IRON_PICKAXE)
            ...
            .setCanClick(false).setClickEvent {
                moveChildInventory(selectIngredientMenu { ingredient ->
                    for (i in craftingSlots){
                        if (getItem(i) != null)continue
                        inv.setItem(i, SItem(ingredient.infoItemStack?:ItemStack(Material.BOOK))
                            .addLore("§cクリックで削除")
        ...
        if (isEdit){
            setItem(40, SInventoryItem(Material.RED_STAINED_GLASS_PANE)
                ...
                .setCanClick(false).setClickEvent {
                    ...
                    SJavaPlugin.sConfig.saveConfig(yml, "recipes/UniqueRecipe/${data!!.internalName}")
    ...
    private fun selectIngredientMenu(onConfirm: Consumer<AbstractIngredient>): LargeSInventory {
        val inv = object : LargeSInventory("SelectIngredient") {
            override fun renderMenu(p: Player): Boolean {
                val items = ArrayList<SInventoryItem>()
                Man10Cooking.ingredients.values.forEach {
                    items.add(SInventoryItem(it.infoItemStack?:ItemStack(Material.BOOK))
                        .addLore(it.internalName)
                        .setCanClick(false)
                        .setClickEvent { _ ->
                            onConfirm.accept(it)

                            this@CreateUniqueRecipeMenu.open(p)
                        })
                }
